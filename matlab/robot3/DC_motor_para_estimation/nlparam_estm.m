...
% <docid:ident_examples.example-ex32807792 Building Structured and User-Defined Models Using System Identification Toolbox™>.
...
z = iddata(y(:,2), u, 1/105, 'Name', 'DC-motor');
...
% more information, see <docid:ident_examples.example-ex27431674 Creating IDNLGREY Model Files>.
%%
%
...
% equation(s).
FileName      = 'dcmotorfirst';       % File describing the model structure.
Order         = [1 1 2];           % Model orders [ny nu nx].
Parameters    = [1*10^-5 20.0 50.0];     
%  Kt, Iw, B,         % Initial parameters. Np = 2.
InitialStates = [0.0 ; 0.0];            % Initial initial states.
Ts            = 1/105;                 % Time-continuous system.
NoiseVariance = 0.1;
nlgr = idnlgrey(FileName, Order, Parameters, InitialStates, Ts, NoiseVariance, 0.1,...
                'Name', 'DC-motor');
nlgr = setinit(nlgr, 'Fixed', {true true}); % Estimate the initial states.
nlgr = setpar(nlgr, 'Fixed', {false false false});
nlgr = setpar(nlgr, 'Maximum' , {Inf Inf Inf});
nlgr = setpar(nlgr, 'Minimum' , {0 0 0});
%%
%
...
nlgr = setinit(nlgr, 'Name', {'Angular velocity' 'Back emf'});
nlgr = setinit(nlgr, 'Unit', {'rad' 'V'});
nlgr = setpar(nlgr, 'Name', {'Kt' 'Iw' 'B'});
nlgr = setpar(nlgr, 'Unit', {'Nm/A' 'kg m^2' 'Nms'});
...
%nlgr = setinit(nlgr, 'Fixed', {true true}); % Estimate the initial states.
%nlgr = setpar(nlgr, 'Fixed', {true true false true false false});
