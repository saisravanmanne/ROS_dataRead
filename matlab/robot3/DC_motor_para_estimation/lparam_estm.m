...
output = [output_A output_B];
input = [control_A*(12.1/400) control_B*(12.1/400)];

%%
% 2. Represent the estimation data as an |iddata| object. 
z = iddata(output, input, 'Name', 'DC-motor');

%%
% 3. Specify input and output signal names, start time and time units. 
z.InputName = {'Voltage_R'; 'Voltage_L'};
z.InputUnit =  {'V';'V'};
z.OutputName = {'Angular velocity_R'; 'Angular Velocity_L'};
...
plot(z(:, :));   % Plot second input-output pair (Voltage -> Angular velocity).


par = [ 0.0337; 1.3023e-04];
aux = {};
Ts = 0;
m = idgrey('lrobot3',par,'c',aux,Ts);
m.Structure.Parameters.Minimum = [0;0];
m.Structure.Parameters.Maximum = [Inf;Inf];
